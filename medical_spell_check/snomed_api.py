...
class SnomedAPI:
    ...
    def search_concepts(self, term: str, limit: int = 5) -> List[Dict]:
        """
        Search for SNOMED CT concepts matching the given term
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"SNOMED API rate limited for term '{term}'")
                return []
            else:
                print(f"SNOMED API error: {response.status_code}")
                return []
                
        except requests.exceptions.Timeout:
            print(f"SNOMED API timeout for term '{term}'")
            return []
        except requests.exceptions.RequestException as e:
            print(f"SNOMED API request error: {e}")
            return []
    ...
    def validate_term(self, term: str) -> bool:
        """
        Validate if a term exists in SNOMED CT
